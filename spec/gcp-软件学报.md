...
We present **Generalized Constraint Projection (GCP)**, a type inference methodology that models each source as an independent constraint dimension (`extendToBe`, `declaredToBe`, `hasToBe`, `definedToBe`) propagated separately via lattice operations and resolved jointly at projection time. We formalize GCP on the **Outline** language, defining a type lattice, a four-dimensional Genericable constraint model with monotonicity and convergence proofs, **Outline Expression Matching (OEM)** as a structural subtyping relation for duck-typed languages, and a bidirectional projection mechanism for polymorphic higher-order function instantiation. We prove soundness (inferred types are safe approximations of runtime types), local and global convergence (fixed-point iteration terminates in O(N·|𝕋|) steps, where N is the number of function parameters and |𝕋| is the type lattice height), and order-independence (inference results are independent of module processing order).

We apply GCP to zero-annotation Python AOT compilation via **GCP-Python**. The `mypyc` compiler can compile type-annotated Python to C extensions with 5–100× speedups, but its practical barrier is *annotation coverage*: unannotated functions yield negligible gains (1.30× average in our integer-intensive benchmarks; one case degrades to 0.81× due to mypyc boxing overhead). GCP-Python is a demand-driven inference pipeline that propagates type constraints from call sites to function parameters, automatically injecting PEP 484 annotations into zero-annotation Python source in **66 ms** of analysis overhead. Evaluated across 22 Python program categories, GCP-Python achieves a **17.1× arithmetic-mean speedup** over CPython, with peaks of **120×** (match/case dispatch), **106×** (default-argument arithmetic), and **34×** on real-world number-theory code (TheAlgorithms/Python). On a 7-function oracle benchmark, GCP-Python reaches **101.8%** of hand-annotated performance on ARM64 and **99.7%** on Linux x86-64 — with zero developer effort — and **outperforms warmed-up Numba JIT** on call-intensive integer functions (`is_prime(997)`: 14.67× vs. 7.66×; `factorial(10)`: 8.53× vs. 2.78×), without code modification or JIT warm-up latency.

**Keywords:** type inference; dynamic languages; constraint solving; ahead-of-time compilation; Python; structural type systems

## 摘要

动态编程语言的类型推导面临一个根本困难:函数参数的类型信息来自四个性质不同、相互独立的来源——函数体内的赋值决定了参数能承载什么值,程序员的显式声明给出了约束上界,调用点传入的实参类型反映了外部对参数的期望,而函数体内的结构化访问(字段读取、方法调用)则揭示了参数必须具备的最小结构。现有方法——从Hindley-Milner统一到TypeScript的双向检查——将这四类信息混入同一个约束集,导致虚假冲突并迫使开发者提供冗余标注。

本文提出**泛化约束投影(Generalized Constraint Projection, GCP)**:将上述四类信息分别建模为独立的约束维度(`extendToBe`、`declaredToBe`、`hasToBe`、`definedToBe`),通过格操作独立传播,在投影时联合求解,而非全局统一。我们在**Outline**语言上形式化该方法,定义类型格、四维Genericable约束模型、作为鸭子类型结构化子类型关系的**Outline表达式匹配(OEM)**,以及用于多态高阶函数实例化的双向投影机制,并证明了健全性、局部与全局收敛性(不动点算法在O(N·|𝕋|)步内终止)以及顺序无关性。

我们将GCP应用于Python零标注AOT编译。`mypyc`可将带类型标注的Python编译为C扩展,但实际瓶颈是*标注覆盖率*:无标注函数几乎无收益(基准测试平均仅1.30×,一个案例因装箱开销退化至0.81×)。我们提出**GCP-Python**:一个需求驱动的推导流水线,从调用点向函数参数传播类型约束,在**66毫秒**内自动为零标注Python源文件注入PEP 484标注。在22个Python程序类别上,GCP-Python相对CPython实现算术平均**17.1倍**加速,峰值达match/case分派**120倍**、默认参数算术**106倍**、真实数论代码(TheAlgorithms/Python)**34倍**。在7函数oracle基准上,ARM64达手工标注性能的**101.8%**,x86-64达**99.7%**,且无需开发者工作;在调用密集型整数函数上**优于预热Numba JIT**(`is_prime(997)`: 14.67× vs 7.66×),无需代码修改且无JIT预热延迟。
...
本文提出了泛化约束投影(GCP)方法论及其在两个不同领域的应用,具体贡献包括:
...
6. **全面评估**(§6):跨22个程序类别、60+基准函数的结果,涵盖Python算术、控制流和函数式模式的全部范围。我们报告CPython、mypyc(bare)、mypyc(GCP)、mypyc(manual-oracle)和Numba(jit)的计时。在7函数核心基准测试中,GCP自动达到手工标注性能的**101.8%**,并在调用密集型模式上**优于预热的Numba JIT**。对TheAlgorithms/Python的真实验证确认了从开源仓库逐字提取的代码平均18.51倍加速。
...
| 槽位 | 符号 | 初始值 | 更新操作 | 收敛方向 |
|------|------|--------|----------|----------|
| `extendToBe` | τ_e | ⊥ (NOTHING) | `τ_e := τ_e ⊔ τ` | 从 ⊥ 向上收敛 |
| `declaredToBe` | τ_d | ⊤ (ANY) | `τ_d := τ` (直接赋值) | 一次性设置 |
| `hasToBe` | τ_h | ⊤ (ANY) | `τ_h := τ_h ⊓ τ` | 从 ⊤ 向下收敛 |
| `definedToBe` | τ_f | ⊤ (ANY) | `τ_f := τ_f ⊓ τ` | 从 ⊤ 向下收敛 |

**约束添加语义:**

```
addExtend(τ):    extendToBe   := extendToBe ⊔ τ   // join: 从 ⊥ 向上扩展,收集所有赋值类型的最小上界
addDeclared(τ):  declaredToBe := τ                 // 直接设置
addDemand(τ):    hasToBe      := hasToBe ⊓ τ       // meet: 从 ⊤ 向下收紧,调用点需求的交集
addStructure(τ): definedToBe  := definedToBe ⊓ τ   // meet: 从 ⊤ 向下收紧,结构访问需求的交集
```

**`hasToBe` 与 `definedToBe` 的合并:**

两者是**并行的上界约束**,分别来自调用点需求和函数体结构访问,投影时必须同时满足。合并为 `hasToBe ⊓ definedToBe`（meet），语义是字段取**并集**——结果比两个操作数都更严格（子类型）:

```
τ_h ⊓ τ_f:
  若 τ_h = ANY                          → τ_f
  若 τ_f = ANY                          → τ_h
  若 τ_h ⊑ τ_f                          → τ_h      (取更严格者,即子类型)
  若 τ_f ⊑ τ_h                          → τ_f      (取更严格者,即子类型)
  若两者均为 ProductADT                  → 结构合并(字段取并集,保留 base)
...
语义：
- `extendToBe ⊑ τ_arg`：实参必须覆盖函数体内所有赋值类型（τ_arg 是 extendToBe 的父类型或相同）
- `τ_arg ⊑ declaredToBe`：实参必须满足程序员的显式声明（τ_arg 是 declaredToBe 的子类型）
- `declaredToBe ⊑ hasToBe ⊓ definedToBe`：声明必须满足调用点需求与结构访问需求的交集
...
│                    Genericable(x)                                       │
│                                                                         │
│  约束链 (左边是子类型/更严格,右边是父类型/更宽松):                         │
│  extendToBe ⊑ τ_arg ⊑ declaredToBe ⊑ hasToBe ⊓ definedToBe            │
│                                                                         │
│  约束来源与收敛方向:                                                      │
│  τ_e ← 函数体内赋值 (x = {name,age,email})  初始⊥,join⊔向上扩展          │
│  τ_d ← 类型声明 (x: Number)                 一次性设置                   │
│  τ_h ← 调用点实参类型传播                   初始⊤,meet⊓向下收紧  ┐        │
│  τ_f ← 函数体内结构访问 (x.name, x.age)     初始⊤,meet⊓向下收紧  ┘⊓合并  │
│                                                                         │
│  NOTHING ⊑ τ (nothing is everything)                                   │
│  τ ⊑ ANY   (everything is any)                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
│        extendToBe = ⊥ ⊔ {name,age,email} = {name,age,email}│
│                                                         │
│ user: Genericable                                       │
...
│ user: Genericable                                       │
│   extendToBe   = {name: String, age: Int, email: String}│
│                  ← join⊔从⊥向上,字段最多(最严格子类型)  │
│   declaredToBe = ⊤ (ANY,未声明)                        │
│   hasToBe      = {email: String}          ┐ meet⊓合并   │
│   definedToBe  = {name: String, age: Int} ┘             │
│                                                         │
│ hasToBe ⊓ definedToBe (字段取并集,更严格):              │
│   {email} ⊓ {name,age} = {name,age,email}               │
│                                                         │
│ 约束链验证:                                             │
│   extendToBe ⊑ declaredToBe ⊑ hasToBe ⊓ definedToBe   │
...
│                                                         │
│ 投影有效范围: τ_arg 满足                                 │
│   {name,age,email} ⊑ τ_arg ⊑ ⊤                         │
│   即 τ_arg 必须是 {name,age,email} 的父类型(字段≤3个)   │
└─────────────────────────────────────────────────────────┘
...
错误示例1: processUser({name: "Charlie", age: 35})
┌─────────────────────────────────────────────────────────┐
│   实参: {name, age}  缺少 email 属性                    │
│   hasToBe ⊓ definedToBe = {name,age,email}              │
│   检查: {name,age,email} ⊑ {name,age} ?                 │
│   ✗ 失败: {name,age} 不是 {name,age,email} 的父类型     │
│     (缺少 email 字段,不满足 hasToBe ⊓ definedToBe)      │
│ 错误报告: 实参缺少必需属性 email                         │
└─────────────────────────────────────────────────────────┘

错误示例2: processUser({name:"Alice", age:35, email:"alice@...", city:"Beijing"})
┌─────────────────────────────────────────────────────────┐
│   实参: {name,age,email,city}  多了 city 属性           │
│   检查: {name,age,email} ⊑ {name,age,email,city} ?      │
│   ✗ 失败: {name,age,email,city} 是 {name,age,email} 的  │
│     子类型(更严格),不满足 extendToBe ⊑ τ_arg            │
│     (τ_arg 比 extendToBe 更严格,超出函数体赋值范围)      │
│ 错误报告: 实参多了属性 city                              │
└─────────────────────────────────────────────────────────┘
```
...
2. **单调收敛**: `extendToBe` 从 ⊥ 向上(join⊔),`hasToBe`/`definedToBe` 从 ⊤ 向下(meet⊓),两端在约束链中间相遇
3. **冲突检测**: 若约束链 `extendToBe ⊑ declaredToBe ⊑ hasToBe ⊓ definedToBe` 不成立,报告类型错误
...
- `addExtend` 通过 join(⊔) 单调递增:τ_e 从 ⊥ 向上收敛（收集所有赋值类型的最小上界）
- `addDemand` 和 `addStructure` 通过 meet(⊓) 单调递减:τ_h、τ_f 从 ⊤ 向下收敛（需求交集越来越严格）
- `declaredToBe` 只设置一次,不参与迭代

**定理3.2 (局部收敛性)**: 对于单个参数,约束序列在有限步内收敛:
- `extendToBe` 从 ⊥ 出发,每次 join 操作单调递增,最多上升到 ⊤,需 O(|𝕋|) 步
- `hasToBe` 和 `definedToBe` 从 ⊤ 出发,每次 meet 操作单调递减,最多下降到 ⊥,需 O(|𝕋|) 步
...
- `extendToBe ⊑ projected(x)`:projected 是 extendToBe 的父类型,覆盖函数体内所有赋值类型
- `projected(x) ⊑ declaredToBe`:projected 满足程序员的显式声明
- `projected(x) ⊑ hasToBe ⊓ definedToBe`:projected 同时满足调用点需求和结构访问需求的交集
...
3. **顶底规则** (NOTHING is everything; everything is ANY):
   - `∀τ. NOTHING ⊑ τ`  — NOTHING 是所有类型的子类型（最严格，可赋给任意变量）
   - `∀τ. τ ⊑ ANY`      — 任何类型都是 ANY 的子类型（最宽松，接受任意值）
   - 等价地：⊥ = NOTHING（格的底元素），⊤ = ANY（格的顶元素）
...
2. **函数体内赋值** - 收集extendToBe约束:
   ...
   在函数体内: x = 42
   如果x是参数:
     addExtend(x, Int)  // x.extendToBe = x.extendToBe ⊔ Int  (join, 从 ⊥ 向上扩展)
   ```
...
3. **使用上下文** - 收集hasToBe约束:
   ...
   如果x是参数:
     addDemand(x, Number)  // x.hasToBe = x.hasToBe ⊓ Number  (meet, 从 ⊤ 向下收紧)
   ```
...
4. **结构化操作** - 收集definedToBe约束:
   ...
   在函数体内: x + 1
   如果x是参数:
     addStructure(x, Number)  // x.definedToBe = x.definedToBe ⊓ Number  (meet, 从 ⊤ 向下收紧)
   ```
...
//   x.hasToBe      = Number    (meet⊓向下收紧)
//   x.definedToBe  = Number    (meet⊓向下收紧)
// 约束检查: Int ⊑ Number ⊑ (Number ⊓ Number) = Number ✓
// (此例两个约束相同,hasToBe ⊓ definedToBe = Number)
...
3. **约束方向**: `extendToBe` 从 ⊥ 向上收敛(join⊔),`hasToBe`/`definedToBe` 从 ⊤ 向下收敛(meet⊓),两端在约束链中间相遇
...
## 4. Python AOT编译应用:GCP-Python

### 4.1 问题分析:mypyc的标注覆盖率问题
...
GCP(泛化约束投影)是围绕四维约束模型构建的类型推导引擎。每个函数参数*x*携带一个约束元组:
...
2. **hasToBe约束**(从调用点向参数传播):
   ...
   - `multiply`的参数`a`: 从调用点3收集 → `hasToBe = ⊤ ⊓ int = int`
   - `multiply`的参数`b`: 从调用点3收集 → `hasToBe = ⊤ ⊓ int = int`
...
4. **extendToBe约束**(从函数体内赋值,此例中无):
   - 这些函数体内没有对参数赋值,所以`extendToBe`保持初始值 ⊥ (NOTHING)
...
步骤2: 处理 add(1.5, 2.5) — 实参 1.5: float
  extendToBe(x) = int ⊔ float = Number
  ┌──────────────────────────────────┐
  │  extendToBe  = Number ← 再次扩展 │
  │  hasToBe     = ⊤                │
  │  definedToBe = ⊤                │
  └──────────────────────────────────┘

步骤3: 分析函数体 x + y — 操作符约束
  definedToBe(x) = ⊤ ⊓ Number = Number
  ┌──────────────────────────────────┐
  │  extendToBe  = Number            │
  │  hasToBe     = ⊤                │
  │  definedToBe = Number ← 向下收紧 │
  └──────────────────────────────────┘

步骤4: 解析 — 约束链交汇
  extendToBe ⊑ τ_arg ⊑ declaredToBe ⊑ hasToBe ⊓ definedToBe
  Number     ⊑ τ_arg ⊑ ⊤            ⊑ ⊤ ⊓ Number = Number
  projected(x) = Number  ✓
...
在类型格中的收敛路径:
         ANY (⊤) ← hasToBe/definedToBe 起点
          │
        Number  ← extendToBe 收敛到此 (join⊔从⊥向上)
        /    \       definedToBe 收敛到此 (meet⊓从⊤向下)
      int   float
        \    /
...
GCP-Python实现为约3000行Java代码,构建在GCP核心引擎之上。
...
### 5.7 RQ5 — 真实世界评估:TheAlgorithms/Python
...
│              │ (静态类型)  │ (动态类型)  │ (动态类型)  │              │
├──────────────┼─────────────┼─────────────┼─────────────┼──────────────┤
│ 工业应用     │ OCaml, F#   │ 广泛采用    │ Python生态  │ Outline,     │
│              │             │ (VS Code等) │             │ GCP-Python   │
└──────────────┴─────────────┴─────────────┴─────────────┴──────────────┘
...
| **动态扩展** | 无原生支持 | `extendToBe`捕获声明后赋值 |
| **渐进类型** | 通过显式`dyn`类型 | 通过`UNKNOWN`/`ANY`初始值 |
| **上下文敏感** | 单态化需要显式泛型 | `projection`自然处理调用点特化 |

**HM需要标注典型案例:从结构访问反向推导参数类型**

HM是名义类型系统,无法从结构访问推导出结构类型。以下函数在HM中无法推导参数类型:

```outline
// 零标注函数
let greet = person -> "Hello, " + person.name + "! Age: " + person.age
...
| 系统 | 推导结果 | 原因 |
|------|---------|------|
| **HM** | 推导失败,需要显式类型声明 | 无结构化子类型,`person.name`无法产生约束 |
| **GCP** | `person: {name: String, age: Int}` ✓ | `definedToBe` 从 `person.name`、`person.age` 收集结构约束;调用点实参满足约束链,投影成功 |
...
**TypeScript需要标注的典型案例:零标注高阶函数的深层类型传播**

```typescript
// TypeScript: 必须显式声明泛型参数和类型
function lift<E, S, R>(
  sel: (e: E) => S,
  pred: (s: S) => R
): (e: E) => R {
  return entity => pred(sel(entity))
}
...
check({name: "Alice", score: 85})  // ✓ 自动推导 entity: {score: Int}

check({name: "Bob", grade: "A"})
// ✗ 投影失败: {grade:String} ⋢ {score:Int}
// GCP精确报告: entity.score 缺失
```

GCP通过三层嵌套的双向投影自动完成:
1. `lift(get_score)` → 推导 `sel: {score:Int} → Int`
2. `lift(get_score)(is_passing)` → 推导 `pred: Int → Bool`
3. `check(entity)` → 反向传播 `entity` 必须有 `{score: Int}`

TypeScript需要程序员手写三个泛型参数 `<E, S, R>`;GCP从调用点和函数体自动收集约束,无需任何标注。
...
本文提出了**泛化约束投影(GCP)**,一种围绕三个核心贡献构建的动态语言类型推导方法论:
...
4. **GCP-Python**展示了GCP约束基底的通用性:同一四维格框架支持类型安全SQL生成,同样支持动态类型语言编译器的需求驱动类型标注。在22个程序类别上,GCP-Python相对CPython实现算术平均**17.1倍**加速,在7函数核心基准上达到手工标注性能的**101.8%**,并在调用密集型整数函数上**优于预热Numba JIT**——无需代码修改且无JIT预热延迟。

5. **Entitir本体平台**将GCP的理论贡献落地于工业规模部署:GCP处理针对实时关系数据库执行的每个实体查询、LLM生成的Outline表达式和决策触发器的类型检查,提供端到端类型安全,防止了整类字段访问错误进入生产环境。GCPToSQL编译器展示了GCP在分析时推导的类型信息足够精确以驱动N-hop外键SQL生成(本文未详细阐述)。
